...
ggplot(data = dailySteps, aes(x = numSteps)) + geom_histogram(fill = "blue", bins = 30) + xlab("Number of Steps") + theme_minimal()
# Mean and Median of number of steps
meanSteps <- mean(na.omit(dailySteps$numSteps))
medSteps <- median(na.omit(dailySteps$numSteps))
ggplot(data = dailySteps, aes(x = "", y = numSteps)) + geom_boxplot(color = "blue") + ylab("Number of Steps") + theme_minimal()
```

* The mean number of steps is `r meanSteps`.
* The median number steps is `r medSteps`.
...
ggplot(data = avData, aes(x = interval, y = avgSteps)) + geom_line(color = "red") + xlab("Interval") + ylab("Number of steps") + ggtitle("Average daily activity pattern") + geom_vline(aes(xintercept = maxInterval), color = "blue") + geom_text(aes(maxInterval,0,label = maxInterval, hjust = 0.5, vjust = 0.5)) + theme_minimal()
```
...
ggplot(data = dailyNoNA, aes(x = steps)) + geom_histogram(fill = "blue", bins = 30) + xlab("Number of Steps")
...
* The mean number of steps is `r meanStepsNoNA`.
* The median number steps is `r medStepsNoNA`.
...
ggplot(data = wdata, aes(x = interval, y = steps, color = wknd)) + geom_line() +facet_wrap(~wknd, nrow = 2)
